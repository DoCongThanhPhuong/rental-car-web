...
export default function AdditionalFunctions({
  ...
  const handleCheckBoxChange = (event) => {
    ...
    const newFunc = checked
      ? [...additionalFunctions, removeWhitespace(value)].filter(
          (func, index, self) =>
            self.findIndex((t) => equalFunc(t, func)) === index
        )
  ...
  const equalFunc = (t1, t2) => {
    return (
      removeWhitespace(t1).toLowerCase() === removeWhitespace(t2).toLowerCase()
    )
  }
